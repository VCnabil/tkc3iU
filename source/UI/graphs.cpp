...
void DoubleSidedVerticalBarGraph::draw2(int position, bool fault) const {
    position = flipped_over_x ? position : position * -1;
...
void DoubleSidedHorizontalBarGraph::draw2(int position, bool fault) const {
    if (flipped_over_y) position = position * -1;
    int width = fullscreen ? 2 : 1;
...
void SingleSidedVerticalBarGraph::draw2(int position, bool fault) const {
    if (flipped_over_y) {
...
void SingleSidedHorizontalBarGraph::draw2(int position) const {
    outimagearea(x_location, y_location, &bar, 0, 0, 5, 10, 100, ALPHA_INVARIANT, LAYER_FRONT);
