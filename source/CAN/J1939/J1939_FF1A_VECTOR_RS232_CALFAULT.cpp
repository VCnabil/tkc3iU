...
void vDECODE_PGN_FF1A_VECTOR_RS232_CALFAULT( unsigned char ucEngineNum, struct buffer_object *rx_tail )
{
    UInt16 uiData = 0;
    int iData = 0;
    ...
    if ( ucDbTimerRefresh( db_VECTOR_cal_fault_error, ucEngineNum, SPECIAL_J1939 ) )
    {
        if ( iData != ( int )data_base[db_VECTOR_cal_fault_error][0].data.flt )
        {
            AlarmMuteFlag = 0;
            uiUnacknowledged_PropulsionSystemFault = 1; // flag bit sent to Indication/Alarm controller
        }

        data_base[db_VECTOR_cal_fault_error][0].data.flt = ( float )iData;
