...
void J1939_FF18_VECTOR_RS232_STA2FAULT(CAN_PORTS_T canPort, CAN_MSG_T* pMsg)
{
	...
	{
		//SetDebugMessage("FF18 RS232_STA2FAULT: Not enough data. Bytes received: %u", pMsg->msg_length);
		return;
	}
	uiData = static_cast<uint32_t>(pMsg->msg_content[0]);

	uint32_t currentValue = 0;
	bool hascurrent = Database_Get_CurrentValue(db_VECTOR_STA2_fault_error, &currentValue);
